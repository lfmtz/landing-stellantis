    ...
    function checkHashAndFocus() {
      ...
      if (targetCard) {
        setTimeout(function() {
          ...
          // Registrar evento en Google Analytics (view_shared_car)
          if (!targetCard.dataset.trackedView) {
            targetCard.dataset.trackedView = 'true';
            try {
              var carNameEl = targetCard.querySelector('.model-name span') || targetCard.querySelector('.model-name');
              var carName = carNameEl ? carNameEl.textContent.trim() : (targetCard.getAttribute('data-id') || 'Auto');
              if (typeof gtag === 'function') {
                gtag('event', 'view_shared_car', {
                  'car_name': carName,
                  'brand_name': '${brand}',
                  'car_id': hash,
                  'source': 'enlace_directo_cliente'
                });
              }
            } catch (e) {}
          }
